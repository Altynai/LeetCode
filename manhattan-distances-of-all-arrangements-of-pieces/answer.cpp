...
class Solution {
public:
  Solution() { comb.init(1e5 + 3); }

  int distanceSum(int n, int m, int k) {
    mint ans = 0;
    // contributions for row distance
    for (int d = 1; d <= n - 1; d++) {
      mint ways = n - d;
      // select x0, x1 with distance d
      // y0 y1 doesn't matter as here we only want row contributions
      ans += ways * d * m * m;
    }
    // contributions for column distance
    for (int d = 1; d <= m - 1; d++) {
      mint ways = m - d;
      ans += ways * d * n * n;
    }
    ans *= comb.choose(n * m - 2, k - 2);
    return int(ans);
  }
};
