itemplate<int MOD_> struct modnum {
...
public:
  ...
  int numberOfWays(int n, int x, int y) {
    ...
    mint score = y;
    mint nums = x; // ways to choose stages with order
    for (int stages = 2; stages <= min(x, n); stages++) {
      nums *= x - stages + 1;
      // choose stages
      mint ways = nums;
      // put N people into M stages (Stirling Number)
      ways *= S[n][stages];
      // add score for each stage
      score *= y;
      ways *= score;
