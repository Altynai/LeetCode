...
public:
  int countKSubsequencesWithMaxBeauty(string s, int k) {
    ...
    int l = k - 1, r = k - 1;
    while (l > 0 and p[l - 1] == p[k - 1])
      l--;
    while (r < n - 1 and p[r + 1] == p[k - 1])
      r++;

    // have to choose all [0, l)
    mint ans = 1;
    for (int i = 0; i < l; i++)
      ans *= p[i];
    ...
    int cand = r - l + 1;
    int chosen = k - l;
    ans *= comb.choose2(cand, chosen);

    // [l, r] has the same frequency
    ans *= mint(p[k - 1]).pow(chosen);
    return int(ans);
  }
};
